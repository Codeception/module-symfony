...
trait EnvironmentAssertionsTrait
{
     ...
     *
     * The version argument supports the same formats accepted by PHP's version_compare,
     * including one, two and three segment versions (for example: `7`, `7.4`, `7.4.8`).
     *
     * ```php
     * <?php
    ...
    /**
     * Asserts that `APP_ENV` and `APP_DEBUG` values match the Kernel state.
     *
    ...
    {
        $appEnv = $this->resolveEnvironmentValue('APP_ENV');
        $appDebug = $this->resolveEnvironmentValue('APP_DEBUG');
        ...
        if ($appDebug !== null) {
            ...
            $normalized = filter_var($appDebug, FILTER_VALIDATE_BOOL, FILTER_NULL_ON_FAILURE);

            $this->assertNotNull(
                $normalized,
                sprintf('APP_DEBUG value "%s" could not be interpreted as a boolean.', $appDebug)
            );
